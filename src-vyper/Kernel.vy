...
getWritePermissions: public(HashMap[bytes5, HashMap[address, bool]])
...
def approvePolicy(policy: address):
    ...
    for permission in permissions:
        self.getWritePermissions[permission][policy] = True
...
def terminatePolicy(policy: address):
    ...
    permissions: DynArray[bytes5, 32] = Policy(policy).requestWrites()
    for permission in permissions:
        self.getWritePermissions[permission][policy] = False
